...
const Orders = () => {
  ...
  const loadOrderData = async () => {
    try {
      ...
      if (!token) {
        console.log("❌ Token not available, exiting function.");
        return;
      }
      ...
      if (res.data.success) {
        ...
        res.data.orders.map((order) => {
          order.items.map((item) => {
            ...
            item['date'] = order.createdAt.split("T")[0];
      ...
      <div>
        {
          orderData.map((item, index) => (
            <div key={index} className='py-4 border-b text-gray-70 flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
